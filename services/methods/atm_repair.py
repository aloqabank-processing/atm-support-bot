...
from api_query import FeedbackModule, MonitoringModel, AdministrationModule
...
async def model_by_filial(call, state):
    ...
    result = await MonitoringModel.get_model_by_state(call.data)
    model = []
    for temp_model in result:
        model.append(temp_model['Model'])
    model = set(model)
...
async def terminal_id_by_model(call, state):
    ...
    terminal_id = await MonitoringModel.get_terminal_id(atm_state, call.data)
    await bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)

...
async def next(FILIAL, call, state):
    ...
    if str(call.message.chat.id) == FILIAL:
        msg = await bot.send_message(chat_id=call.message.chat.id, text=language['20'], reply_markup=keyboard.choose_problem(language['18'], language['17'], language['19'], language['34']))
        await state.update_data(button=msg.message_id)
    else:
